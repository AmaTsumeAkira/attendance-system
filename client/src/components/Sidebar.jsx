...
import {
  ...
  CalendarCheck, FileText, Calendar, User, LogOut, Monitor, Bell, CalendarRange
} from 'lucide-react'
    ...
    <nav className="sidebar-nav">
      ...
      {(isAdmin || isTeacher) && (
        <>
          ...
          {isAdmin && <Link to="/semesters" icon={CalendarRange}>学期管理</Link>}
          ...
          {isAdmin && <Link to="/dashboard-screen" icon={Monitor}>数据大屏</Link>}
        </>
      )}
      ...
      <Link to="/notifications" icon={Bell}>通知中心</Link>
